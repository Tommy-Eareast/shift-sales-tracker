...
export function Modal({ isOpen, onClose, title, subtitle, children, persistent = true }: ModalProps) {
    useEffect(() => {
        if (isOpen) {
    ...
    return (
        ...
        >
            ...
            >
                <div className="w-10 h-1 rounded-full mx-auto mb-5" style={{ backgroundColor: '#e7e5e2' }} />

                {title && <h3 className="text-lg font-semibold text-stone-900 mb-1 tracking-tight">{title}</h3>}
