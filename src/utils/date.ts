 ...
 */
export function formatShiftDisplayName(recordDate: string, shiftTimeStart: string, shiftTimeEnd: string): string {
 ...
 */
export function formatExportFilename(recordDate: string, shiftTimeStart: string, shiftTimeEnd: string): string {
    const [year, month, day] = recordDate.split('-');
    const date = new Date(parseInt(year), parseInt(month) - 1, parseInt(day));
    const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
