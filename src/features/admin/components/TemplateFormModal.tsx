...
type Props = {
    isOpen: boolean;
    onClose: () => void;
    editingTemplate: { templateId: string; templateName: string; brandList: string[] } | null;
    form: { templateName: string; selectedBrands: string[] };
    onChange: (form: { templateName: string; selectedBrands: string[] }) => void;
    error: string;
    brands: string[];
    onSave: () => void;
};
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <input
                        ...
                        onChange={e => onChange({ ...form, templateName: e.target.value })}
                        className={inputClass}
                    />
                </div>
                <div>
                    <label className="block text-xs font-medium text-stone-500 mb-2 uppercase tracking-wider">
                        Select Brands <span className="text-stone-300 font-normal">(optional)</span>
                    </label>
                    <p className="text-xs text-stone-300 mb-3">
                        Choose brands for this template. You can add products to these brands later.
                    </p>
                    {brands.length === 0 ? (
                        <p className="text-sm text-stone-400 p-4 bg-stone-50 rounded-xl text-center">
                            No brands available. Add products first or create the template without brands.
                        </p>
                    ) : (
                                ...
                                <label
                                    ...
                                    style={
                                        form.selectedBrands.includes(brand)
                                            ? { backgroundColor: '#ecf5f1', borderColor: '#5b8c7a' }
                                            : {}
                                    }
                                >
                                    <input
                                        type="checkbox"
                                        checked={form.selectedBrands.includes(brand)}
                                        onChange={() =>
                                            onChange({
                                                ...form,
                                                selectedBrands: form.selectedBrands.includes(brand)
                                                    ? form.selectedBrands.filter(b => b !== brand)
                                                    : [...form.selectedBrands, brand],
                                            })
                                        }
                                        className="w-4 h-4 rounded"
                                        style={{ accentColor: '#5b8c7a' }}
                                    />
                                    <span className="text-sm text-stone-700 select-none">{brand}</span>
                                </label>
                            ))}
                        </div>
                    )}
